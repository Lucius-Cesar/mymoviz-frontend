...
function Home() {
  ...
  useEffect(() => {
    fetch('https://mymoviz-backend-ivory.vercel.app/movies')
 ...
 const movies = moviesData.map((data, i) => {
  const imgUrlFirstPart = "https://image.tmdb.org/t/p/w440_and_h660_face/"
  const isLiked = likedMovies.some(movie => movie === data.title);
  return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview.slice(0, 249)+"..."} poster={imgUrlFirstPart+data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
 });
